<div class="container">
<h1 style="text-align: center"> Best Seller </h1>
<br>
<p style="text-align:center">Our best selling items</p>
<br>
<br>
<ul class="nav nav-pills" style="text-align: center;margin:0 auto;width:58%">
    <li class=" nav-item  "><a class="nav-link active" href="#">ALL</a></li>
    <li  class=" nav-item "><a class="nav-link" href="#">Business Card</a></li>
    <li  class=" nav-item" ><a class="nav-link" href="#">Stationary</a></li>
    <li  class=" nav-item "><a class="nav-link" href="#">Packaging</a></li>
    <li  class=" nav-item "><a class="nav-link" href="#">Marketing Materials</a></li>
  </ul>
  <br>
  <br>
  <div class="row" style="display:flex">
  <div class="card col-md-3 col-sm-6 col-xs-12 mb-3" style="width:400px;border:0px">
    <img class="card-img-top" src="image/paperbag.jpg" alt="Card image">
    <div class="card-body">
      <a href="#">3 Color 170gsm Art Paper Large Wide Shopping Bag</a>
    </div>
  </div>

  <div class="card col-md-3 col-sm-6 col-xs-12 mb-3" style="width:400px;border:0px">
    <img class="card-img-top" src="image/paperbag.jpg" alt="Card image">
    <div class="card-body">
      <a href="#">3 Color 170gsm Art Paper Large Wide Shopping Bag</a>
    </div>
  </div>

  <div class="card col-md-3 col-sm-6 col-xs-12 mb-3" style="width:400px;border:0px">
    <img class="card-img-top" src="image/paperbag.jpg" alt="Card image">
    <div class="card-body">
      <a href="#" >3 Color 170gsm Art Paper Large Wide Shopping Bag</a>
    </div>
  </div>
  <div class="card col-md-3 col-sm-6 col-xs-12 mb-3" style="width:400px;border:0px">
    <img class="card-img-top" src="image/paperbag.jpg" alt="Card image">
    <div class="card-body">
      <a href="#">3 Color 170gsm Art Paper Large Wide Shopping Bag</a>
    </div>
  </div>
  </div>
</div>
  